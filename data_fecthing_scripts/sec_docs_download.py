...
PRIMARY_HTML_HINT = re.compile(r"(10-k|10q|10-q|8-k|8k|primary|main|document|form10)", re.I)
...
def throttle(): time.sleep(1.0 / MAX_RPS)
...
def no_leading_zeros(cik: str) -> str:
    ...
    return str(int(cik))
def acc_nodash(acc: str) -> str: return acc.replace("-", "")
...
def guess_primary_html(files: List[FileItem]) -> Optional[str]:
    ...
    htmls = [f for f in htmls if not f.name.lower().startswith(("ex-", "ex_", "ex"))]  # skip exhibits
...
def choose_minimal(files: List[FileItem],
                   include_primary=True,
                   include_ex99=True,
                   include_finrep=True) -> List[Tuple[str, str]]:
    """Return [(name, kind)] where kind in {'primary','ex99','financial_report'}"""
    chosen = []
    if include_primary:
        prim = guess_primary_html(files)
        if prim: chosen.append((prim, "primary"))
    if include_ex99:
        for f in files:
            if EX99_RE.search(f.name): chosen.append((f.name, "ex99"))
    if include_finrep:
        for f in files:
            if FINREP_RE.search(f.name): chosen.append((f.name, "financial_report"))
    # dedupe, keep order
    seen = set(); out=[]
    for n,k in chosen:
        if n not in seen:
            seen.add(n); out.append((n,k))
    return out
...
def bytes_to_text(name: str, data: bytes) -> Tuple[str, str]:
    ...
    try: return data.decode("utf-8", errors="ignore"), "text/plain"
    except: return "", "application/octet-stream"

def run(input_csv: str, docs_out_csv: str,
        include_primary: bool, include_ex99: bool, include_finrep: bool):
    with open(input_csv, newline="") as fh:
        rows = list(csv.DictReader(fh))

    with open(docs_out_csv, "w", newline="") as out:
        w = csv.writer(out)
        w.writerow(["cik","accession_number","name","url","kind","content_type","text"])
        for row in rows:
            ...
            picks = choose_minimal(files, include_primary, include_ex99, include_finrep)
...
if __name__ == "__main__":
    import io, re
    ap = argparse.ArgumentParser(description="Download only primary HTML, EX-99 exhibits, and Financial_Report.* from EDGAR index.json -> docs_text.csv")
    ap.add_argument("--input", required=True, help="CSV with columns: cik,accession_number")
    ap.add_argument("--docs-out", required=True, help="Output CSV with extracted text (filtered files only)")
    ...
    args = ap.parse_args()
    run(args.input, args.docs_out, include_primary=not args.no_primary, include_ex99=not args.no_ex99, include_finrep=not args.no_finrep)
